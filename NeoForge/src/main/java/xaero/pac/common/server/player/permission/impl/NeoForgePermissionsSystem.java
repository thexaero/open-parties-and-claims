...
public class NeoForgePermissionsSystem implements IPlayerPermissionSystemAPI {

	private static final Map<IPermissionNodeAPI<?>, PermissionNode<?>> REGISTERED_NODES = new HashMap<>();

	public NeoForgePermissionsSystem(){
	}
	...
	public static void registerNodes(PermissionGatherEvent.Nodes event){
		UsedPermissionNodes.ALL.values().stream().map(NeoForgePermissionsSystem::convertNode).filter(Objects::nonNull).map(NeoForgePermissionsSystem::onRegister).forEach(event::addNodes);
	}
