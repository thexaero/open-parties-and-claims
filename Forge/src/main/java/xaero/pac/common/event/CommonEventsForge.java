...
public class CommonEventsForge extends CommonEvents {
	...
	public void onEntityMultiPlaceBlock(BlockEvent.EntityMultiPlaceEvent event) {
		if(super.onEntityMultiPlaceBlock(event.getLevel(), event.getReplacedBlockSnapshots().stream().map(s -> Triple.of(s.getPos(), s.getReplacedBlock(), s.getCurrentBlock())), event.getEntity()))
			event.setCanceled(true);
	}
	...
	public void onAddonRegister(OPACServerAddonRegisterEvent event){
		...
		event.getContext().getPermissionSystemManagerAPI().register("permission_api", new ForgePermissionsSystem());
	}
	...
	public void fireAddonRegisterEvent(OPACServerAddonRegisterEventContext context, IServerData<IServerClaimsManager<IPlayerChunkClaim, IServerPlayerClaimInfo<IPlayerDimensionClaims<IPlayerClaimPosList>>, IServerDimensionClaimsManager<IServerRegionClaims>>, IServerParty<IPartyMember, IPartyPlayerInfo, IPartyAlly>> serverData) {
		MinecraftForge.EVENT_BUS.post(new OPACServerAddonRegisterEvent(context));

		//TODO remove this when the deprecated event is removed
		MinecraftForge.EVENT_BUS.post(new xaero.pac.common.event.api.OPACServerAddonRegisterEvent(serverData.getServer(), serverData.getPlayerPermissionSystemManager(), serverData.getPlayerPartySystemManager(), serverData.getServerClaimsManager().getTracker()));
	}
