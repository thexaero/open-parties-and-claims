...
public class MainMenu extends XPACScreen {
	...
	protected void init() {
		super.init();
		addRenderableWidget(configsButton = new Button(width / 2 - 100, height / 8 + 8, 200, 20, new TranslatableComponent("gui.xaero_pac_ui_config_menu"), this::onConfigsButton));
		
		aboutPartyButton = new Button(width / 2 - 100, height / 8 + 40, 70, 20, new TranslatableComponent("gui.xaero_pac_ui_about_party"), this::onAboutPartyButton, new Button.OnTooltip() {
	...
	private DropDownWidget setupClaimModeDropdown(){
		...
		return DropDownWidget.Builder.begin()
				...
				.setNarrationTitle(new TranslatableComponent("gui.xaero_pac_claiming_as_narration"))
				.build();
	}
	...
	private boolean onClaimMode(DropDownWidget dropDownWidget, int index) {
		...
		sendMessage("/" + ClaimsCommandRegister.COMMAND_PREFIX + " " + selectedClaimingMode.getId() + "-claim-mode");
