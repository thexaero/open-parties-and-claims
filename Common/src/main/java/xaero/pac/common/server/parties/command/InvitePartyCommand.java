		...
		LiteralArgumentBuilder<CommandSourceStack> command = Commands.literal(PartyCommandRegister.COMMAND_PREFIX).requires(c -> ServerConfig.CONFIG.partiesEnabled.get()).then(Commands.literal("member")
				...
				.then(Commands.argument("player", EntityArgument.player())
						...
						.executes(context -> {
							...
							Component callerName = Component.literal(player.getGameProfile().name()).withStyle(ChatFormatting.DARK_GREEN);
							Component targetName = Component.literal(targetPlayer.getGameProfile().name()).withStyle(ChatFormatting.YELLOW);
							...
							acceptComponent.getSiblings().add(adaptiveLocalizer.getFor(targetPlayer, "gui.xaero_parties_invite_target_message_accept").withStyle(s -> s.withColor(ChatFormatting.GREEN).withClickEvent(new ClickEvent.RunCommand("/" + PartyCommandRegister.COMMAND_PREFIX + " join " + playerParty.getId())).withHoverEvent(new HoverEvent.ShowText(adaptiveLocalizer.getFor(targetPlayer, "gui.xaero_parties_invite_target_message_accept_tooltip")))));
							...
							new PartyOnCommandUpdater().update(player, serverData, playerParty, serverData.getPlayerConfigManager(), mi -> false, Component.translatable("gui.xaero_parties_invite_party_message", callerName, targetName));
							return 1;
						}))));
