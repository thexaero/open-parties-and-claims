		...
		LiteralArgumentBuilder<CommandSourceStack> command = Commands.literal(PartyCommandRegister.COMMAND_PREFIX).requires(c -> ServerConfig.CONFIG.partiesEnabled.get()).then(Commands.literal("ally")
				...
				.then(Commands.argument("player", EntityArgument.player())
						...
						.executes(context -> {
							...
							new PartyOnCommandUpdater().update(player, serverData, targetPlayerParty, serverData.getPlayerConfigManager(), mi -> false, new TranslatableComponent("gui.xaero_parties_ally_player_target_party_info", new TextComponent(playerParty.getDefaultName()).withStyle(s -> s.withColor(ChatFormatting.DARK_GREEN)), targetPlayerParty.getDefaultName()));

							Component callerName = new TextComponent(player.getGameProfile().getName()).withStyle(ChatFormatting.DARK_GREEN);
							Component targetPartyName = new TextComponent(targetPlayerParty.getDefaultName()).withStyle(ChatFormatting.YELLOW);
							new PartyOnCommandUpdater().update(player, serverData, playerParty, serverData.getPlayerConfigManager(), mi -> false, new TranslatableComponent("gui.xaero_parties_ally_caster_party_info", callerName, targetPartyName));
							return 1;
						}))));
