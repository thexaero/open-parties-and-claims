		...
		LiteralArgumentBuilder<CommandSourceStack> command = Commands.literal(PartyCommandRegister.COMMAND_PREFIX).requires(c -> ServerConfig.CONFIG.partiesEnabled.get()).then(Commands.literal("ally")
				...
				.then(Commands.argument("owner", StringArgumentType.word())
						...
						.executes(context -> {
							...
							Component targetName = new TextComponent(targetPlayerParty.getDefaultName()).withStyle(ChatFormatting.YELLOW);
							
							new PartyOnCommandUpdater().update(player, serverData, targetPlayerParty, serverData.getPlayerConfigManager(), mi -> false, new TranslatableComponent("gui.xaero_parties_unally_target_party_message", new TextComponent(playerParty.getDefaultName()).withStyle(s -> s.withColor(ChatFormatting.DARK_GREEN)), targetName));

							Component callerName = new TextComponent(player.getGameProfile().getName()).withStyle(ChatFormatting.DARK_GREEN);

							new PartyOnCommandUpdater().update(player, serverData, playerParty, serverData.getPlayerConfigManager(), mi -> false, new TranslatableComponent("gui.xaero_parties_unally_caster_party_message", callerName, targetName));
							return 1;
						}))));
