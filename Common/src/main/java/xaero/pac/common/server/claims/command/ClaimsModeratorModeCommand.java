...
public class ClaimsModeratorModeCommand {
		...
		LiteralArgumentBuilder<CommandSourceStack> command = Commands.literal(ClaimsCommandRegister.COMMAND_PREFIX).requires(context -> ServerConfig.CONFIG.claimsEnabled.get()).then(Commands.literal("moderator-mode")
				.requires(CommandRequirementHelper.onServerThread(context -> {
						...
						try {
							...
							if(serverData.getServerClaimsManager().getPermissionHandler().playerHasModeratorModePermission(player))
								return true;
							return ServerPlayerDataAPI.from(player).isClaimsModeratorMode();//lets you turn off moderator mode even if you're not op
				...
				.executes(context -> {
					...
					playerData.setClaimsAdminMode(false);
