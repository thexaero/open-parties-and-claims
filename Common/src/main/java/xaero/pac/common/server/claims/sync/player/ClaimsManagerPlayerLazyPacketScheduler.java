...
public abstract class ClaimsManagerPlayerLazyPacketScheduler implements ILazyPacketScheduleTask {
	...
	@Override
	public final void onTick(IServerData<?,?> serverData, ServerPlayer player, int limit) {
		if(shouldWork())
			doSchedule(serverData, player, limit);
	}

	protected abstract void doSchedule(IServerData<?,?> serverData, ServerPlayer player, int limit);
