...
//Exists so that additions to playerConfigurablePlayerConfigOption can be done in the right order relative to
//other player config changes, such as renames.
public class PlayerConfigConfigurableAdder implements IPlayerConfigTransformer, IPlayerConfigConfigurableTransformer {

	private final ImmutableList<String> toAdd;

	public PlayerConfigConfigurableAdder(ImmutableList<String> toAdd) {
		this.toAdd = toAdd;
	}
	...
	public void transform(Config configData, Set<String> keys) {
		//does nothing because only the server config option playerConfigurablePlayerConfigOption needs changes
	}

	@Override
	public boolean transformConfigurableList(List<String> playerConfigurableOptions) {
		playerConfigurableOptions.addAll(toAdd);
		return true;
	}
