		...
		LiteralArgumentBuilder<CommandSourceStack> command = Commands.literal(PartyCommandRegister.COMMAND_PREFIX).requires(c -> ServerConfig.CONFIG.partiesEnabled.get()).then(Commands.literal("member")
				...
				.then(Commands.argument("name", StringArgumentType.word())
						...
						.executes(context -> {
							...
							Component targetName = new TextComponent(targetPlayerInfo.getUsername()).withStyle(ChatFormatting.YELLOW);

							new PartyOnCommandUpdater().update(player, serverData, playerParty, serverData.getPlayerConfigManager(), mi -> false, new TranslatableComponent("gui.xaero_parties_rank_party_message", callerName, targetName, new TextComponent(targetRank.toString()).withStyle(s -> s.withColor(targetRank.getColor()))));
