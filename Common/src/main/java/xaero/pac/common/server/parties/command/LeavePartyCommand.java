				...
				.executes(context -> {
					...
					if(playerParty.getOwner().getUUID().equals(contextPlayerId)) {
						...
						if(serverPlayerData.getPartiesImpersonatedPlayerId() == null)
							message = new TranslatableComponent("gui.xaero_parties_leave_party_message", new TextComponent(memberToRemove.getUsername()).withStyle(s -> s.withColor(ChatFormatting.YELLOW)));
						else
