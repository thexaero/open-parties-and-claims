		...
		return context -> {
				...
				ClaimResult<?> result = null;
			 	try {
					...
					result = claimsManager.tryToForceloadTyped(world.dimension().identifier(), claimPlayerId, fromDimension, fromX, fromZ, middleX, middleZ, enable, shouldReplace);
					...
					if(!result.getResultType().success) {
				 	...
				 	if(enable)
						context.getSource().sendSuccess(adaptiveLocalizer.supplierFor(player, "gui.xaero_claims_forceloaded_at", middleX, middleZ, world.dimension().identifier().toString()), true);
				 	else
						context.getSource().sendSuccess(adaptiveLocalizer.supplierFor(player, "gui.xaero_claims_unforceloaded_at", middleX, middleZ, world.dimension().identifier().toString()), true);
				 	return 1;
			 	} finally {
					 if(result != null && player != null) {
						 ...
						 ((ClaimsManagerSynchronizer) claimsManager.getClaimsManagerSynchronizer()).syncToPlayerClaimActionResult(
								 new AreaClaimResult(Sets.newHashSet(result.getResultType()), customReasons, world.dimension().identifier(), areaLeft, areaTop, areaRight, areaBottom),
								 player);
					 }
			 	}
			};
