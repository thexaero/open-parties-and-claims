...
public class ClaimsClaimCommands {
		...
		return context -> {
				...
				try {
					...
					if(shouldClaim) {
						...
						result = claimsManager.tryToUnclaimTyped(world.dimension().location(), claimPlayerId, fromDimension, fromX, fromZ, middleX, middleZ, shouldReplace);
						if(!result.getResultType().success) {
							Component message = adaptiveLocalizer.getFor(player, result.getMessage());
							context.getSource().sendFailure(message);
							return 0;
	...
	public static void sendResult(
		...
		if(customReasons.hasNext())
			sourceStack.sendSuccess(adaptiveLocalizer.getFor(player, "gui.xaero_claims_claim_action_forbidden_by_addon_reasons"), true);
		int reasonNumber = 0;
