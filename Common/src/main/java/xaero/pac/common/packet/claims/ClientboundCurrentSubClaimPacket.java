		...
		public ClientboundCurrentSubClaimPacket apply(FriendlyByteBuf input) {
			try {
				...
				for (String modeId : subInfoTag.keySet()) {
					...
					CompoundTag modeSubInfoTag = subInfoTag.getCompoundOrEmpty(modeId);
					int currentSubConfigIndex = modeSubInfoTag.getIntOr("i", 0);
					String currentSubConfigId = modeSubInfoTag.getStringOr("s", "");
